...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    # 1. Figure out the actual rate by adding the spread percentage fee
    actual_rate = exchange_rate * (1 + (spread / 100))
    
    # 2. Convert your budget into the foreign currency
    total_foreign_money = exchange_money(budget, actual_rate)
    
    # 3. Figure out how many whole bills you can get
    number_of_bills = get_number_of_bills(total_foreign_money, denomination)
    
    # 4. Turn those bills back into their total cash value
